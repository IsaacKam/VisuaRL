...
class mask_encoder(nn.Module):
    def __init__(self, nIn=3, nf=64, nOut=8):
        ...
        #             init_(nn.Conv2d(3,10, 7, stride=2)), nn.ReLU(),
        #             init_(nn.Conv2d(10,5, 4, stride=1)), nn.ReLU(),
        #             init_(nn.Conv2d(5,4,4, stride=1)), nn.ReLU(), 
        #             init_(nn.Conv2d(4,2,3, stride=1)), nn.ReLU(),Flatten(),
        #             init_(nn.Linear(5618,512)), nn.ReLU()
        # )
        self.encoder = nn.Sequential(
            init_(nn.Conv2d(2,10, 7, stride=3)), nn.ReLU(),
            init_(nn.Conv2d(10,20, 4, stride=2)), nn.ReLU(),
            init_(nn.Conv2d(20,10,3, stride=1)), nn.ReLU(), 
            init_(nn.Conv2d(10,5,3, stride=1)), nn.ReLU(),Flatten(),
            init_(nn.Linear(1125,512)), nn.ReLU()
            )
    def forward(self, x):
